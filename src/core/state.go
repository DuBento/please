...
// parsingDependencies returns the dependencies of a target including its adjacent targets generated by the same statement.
func (state *BuildState) parsingDependencies(target *BuildTarget) BuildLabels {
	deps := target.DeclaredDependencies()
	pkg := state.Graph.PackageByLabel(target.Label)
	if pkg == nil {
		return deps
	}
	deps = append(deps, pkg.Metadata.FindRelatedTargets(target)...)
	return deps
}
...
func (state *BuildState) queueTargetAsync(target *BuildTarget, forceBuild, building bool, mode ParseMode) {
	...
	for _, dep := range state.parsingDependencies(target) {
