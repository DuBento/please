...
func newInterpreter(state *core.BuildState, p *Parser) *interpreter {
	s := &scope{
		...
		origins: map[string]core.BuildLabel{},
		used:    map[core.BuildLabel]bool{},
	}
	i := &interpreter{
		scope:      s,
		parser:     p,
...
type scope struct {
	...
	cursor *Statement
	// origins tracks the subinclude label that each variable was originally defined in.
	origins map[string]core.BuildLabel
	// used tracks which subinclude labels have been used by looking up variables.
	used map[core.BuildLabel]bool
}
...
func (s *scope) Set(name string, value pyObject) {
	s.locals[name] = value
	if s.subincludeLabel != nil {
		s.origins[name] = *s.subincludeLabel
	}
}
...
func (s *scope) ActiveSubincludes() core.SubincludesLabelProvider {
	return func() core.BuildLabels {
		...
		for callScope := s; callScope != nil; callScope = callScope.caller {
			for label := range callScope.used {
				seen[label] = true
			}
		}
